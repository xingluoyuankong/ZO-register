...
async function main() {
  ...
  const context = await chromium.launchPersistentContext(
    join(homedir(), 'AppData', 'Local', 'zo-check'),
  ...
  log(`\n✅ link`);
  if (rt !== acc.refreshToken) { acc.refreshToken = rt; writeFileSync(ACCOUNTS_FILE, JSON.stringify([acc], null, 2), 'utf-8'); }
  ...
  for (let a = 0; a < 10; a++) {
    ...
    if (host.endsWith('.zo.computer') && host !== 'www.zo.computer') { log(`✅ ${host}`); break; }
  ...
  // ★ 检查保活状态 - 直接在终端输入
  async function runCheck(cmd, desc) {
    log(`\n[${desc}]`);
    // 尝试多种方式找终端输入
    await sleep(3000);
    // 直接在终端中输入（终端可能由xterm等组件渲染）
  ...
  await runCheck('ps aux | grep -v grep | grep keepalive || echo "NOT_RUNNING"', '保活进程');
  await runCheck('cat /tmp/keepalive.log 2>/dev/null | tail -10 || echo "NO_LOG"', '保活日志');
  await runCheck('cat /tmp/keepalive_state.json 2>/dev/null || echo "NO_STATE"', '保活状态');

  // ★ 修复：把面板部署到ZO子域名下
  // ZO用nginx反向代理，80端口serve子域名
  log('\n===== 部署面板到ZO子域名 =====');
  
  // 停止保活，更新脚本用80端口
  await runCheck('pkill -f keepalive.js 2>/dev/null; sleep 2; echo done', '停止旧保活');
  
  // 下载最新保活脚本
  await runCheck('curl -fsSL -o /home/user/keepalive.js https://raw.githubusercontent.com/xingluoyuankong/ZO-register/master/keepalive_full_puppet.js && echo "UPDATED"', '更新脚本');
  
  // 用sudo在80端口启动（ZO子域名走80端口）
  await runCheck('cd /home/user && sudo nohup xvfb-run -a node keepalive.js > /tmp/keepalive.log 2>&1 & echo "PID=$!"', '80端口启动');
  
  // 验证
  await runCheck('sleep 10 && curl -s localhost:80 | head -5 || echo "PANEL_FAIL"', '验证面板');
  await runCheck('ps aux | grep -v grep | grep keepalive', '验证进程');

  log('\n===== 部署完成 =====');
  log(`面板地址: https://${acc.handle}.zo.computer (ZO子域名)`);
  log(`内部: curl localhost:80`);
  log(`API: curl localhost:80/api/state`);
